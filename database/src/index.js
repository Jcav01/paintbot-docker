...
app.get('/sources/:notificationSource', asyncHandler(async (req, res) => {
...
app.post('/source', asyncHandler(async (req, res) => {
...
// Get list of notification types for a specific notification source
app.get('/notifications/types/:notificationSource', asyncHandler(async (req, res) => {
...
app.post('/notifications/history', asyncHandler(async (req, res) => {
...
app.get('/notifications/history/info', asyncHandler(async (req, res) => {
	...
	const result = await db.query('SELECT * FROM past_notifications WHERE notification_info @> $1::jsonb ORDER BY received_date DESC LIMIT 1', [req.query.search]);
	res.json(result.rows);
}));

app.get('/notifications/history/:source', asyncHandler(async (req, res) => {
...
app.get('/notifications/history/:source/:type', asyncHandler(async (req, res) => {
...
app.get('/destinations/source/:source', asyncHandler(async (req, res) => {
...
app.get('/destinations/channel/:channel', asyncHandler(async (req, res) => {
...
app.put('/destinations/:destination/:source', asyncHandler(async (req, res) => {
...
app.post('/destination', asyncHandler(async (req, res) => {
	...
	if (source.rows.length === 0) {
		...
		try {
			...
			await client.query('INSERT INTO sources (source_id, notification_source, source_username) VALUES($1, $2, $3)', [req.body.source_id, req.body.notification_source, req.body.source_username]);
			result = await client.query('INSERT INTO destinations (channel_id, source_id, minimum_interval, highlight_colour, notification_message) VALUES($1, $2, $3, $4, $5) RETURNING channel_id', [req.body.channel_id, req.body.source_id, req.body.minimum_interval, req.body.highlight_colour, req.body.message]);
			await client.query('COMMIT');
		} catch (error) {
			await client.query('ROLLBACK');
			console.log('Error adding destination:', error);
			return res.status(500).send({message: 'Unable to add destination! No changes were made.'});
		} finally {
			client.release();
		}
	} else {
...
app.delete('/destination/:destination/:source', asyncHandler(async (req, res) => {
	console.log('Received request to remove destination:', req.params.destination, 'for source:', req.params.source);
	const client = await db.getClient();
	try {
		await client.query('BEGIN');
		await client.query('DELETE FROM destinations WHERE channel_id = $1 AND source_id = $2', [req.params.destination, req.params.source]);
		...
		return res.status(500).send({message: 'Unable to delete destination! No changes were made.'});
	} finally {
		client.release();
	}
	res.send();
