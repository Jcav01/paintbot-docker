      ...
      subcommand
        ...
        .addNumberOption((option) =>
          option
            .setName('interval')
            .setDescription(
              'The minimum interval (in minutes) between notifications. Default is 15.'
            )
            .setRequired(false)
        )
        .addStringOption((option) =>
          option
            .setName('highlight')
            .setDescription(
              'RGB hex code for the colour on the left of the embed. Default is 9146FF.'
            )
            .setRequired(false)
        )
        .addStringOption((option) =>
      ...
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName('highlight')
            .setDescription(
              'RGB hex code for the colour on the left of the embed. Default is CD201F.'
            )
            .setRequired(false)
        )
        .addStringOption((option) =>
  ...
  async execute(interaction) {
    ...
    if (interaction.options.getSubcommand() === 'twitch') {
      ...
      const req = http.request(request_options, async (res) => {
        if (res.statusCode !== 200) {
          await interaction.editReply({
            content: 'Failed to add Twitch notification. No changes have been made.',
          });
          return;
        }
        await interaction.editReply({
          content: `Added Twitch notification for ${interaction.options.getString('channel')}.`,
        });
      });
      req.write(options);
      req.end();
      ...
      const req = http.request(request_options, async (res) => {
        if (res.statusCode !== 200) {
          await interaction.editReply({
            content: 'Failed to add Youtube notification. No changes have been made.',
          });
          return;
        }
        await interaction.editReply({
          content: `Added Youtube notification for ${interaction.options.getString('channel')}.`,
        });
      });
      req.write(options);
      req.end();
