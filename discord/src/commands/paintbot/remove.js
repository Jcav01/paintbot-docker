  ...
  async execute(interaction) {
    ...
    if (interaction.options.getSubcommand() === 'twitch') {
      ...
      const req = http.request(request_options, async (res) => {
        if (res.statusCode !== 200) {
          await interaction.editReply({
            content: 'Failed to remove Twitch notification. No changes have been made.',
          });
          return;
        }
        await interaction.editReply({
          content: `Removed Twitch notification for ${interaction.options.getString('channel')}.`,
        });
      });
      req.write(options);
      req.end();
      ...
      const req = http.request(request_options, async (res) => {
        if (res.statusCode !== 200) {
          await interaction.editReply({
            content: 'Failed to remove Youtube notification. No changes have been made.',
          });
          return;
        }
        await interaction.editReply({
          content: `Removed Youtube notification for ${interaction.options.getString('channel')}.`,
        });
      });
      req.write(options);
      req.end();
