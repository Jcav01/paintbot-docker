  ...
  apiClient.users.getUserByName(req.body.source_username).then(async (user) => {
    ...
    const source_req = http.request(options, (source_res) => {
      ...
      apiClient.eventSub.getSubscriptions().then((subs) => {
        console.log(
          `Subscription quota: ${subs.totalCost} / ${subs.maxTotalCost}`
        );
      });
  ...
  apiClient.users.getUserByName(req.body.source_username).then(async (user) => {
    ...
    const delete_req = http.request(options, (destination_res) => {
      ...
      if (destination_res.statusCode !== 200) {
        res
          .status(destination_res.statusCode)
          .send({ message: destination_res.message });
        return;
      }
...
try {
  ...
  secrets = {
    ...
    clientSecret: fs
      .readFileSync(`${secretsPath}/client-secret`, "utf8")
      .trim(),
    eventSubSecret: fs
      .readFileSync(`${secretsPath}/eventsub-secret`, "utf8")
      .trim(),
  };
...
async function handleStreamOnline(broadcasterId) {
  ...
  const destinationRes = await fetch(
    `http://database:8002/destinations/source/${broadcasterId}`
  );
  ...
  const lastNotifRes = await fetch(
    `http://database:8002/notifications/history/${broadcasterId}/stream.offline`
  );
...
function addEvents(sourceId) {
  subs.push({
    source: sourceId,
    subscriptions: [
      twitchListener.onStreamOnline(sourceId, (e) =>
        handleStreamOnline(e.broadcasterId)
      ),
      twitchListener.onStreamOffline(sourceId, (e) =>
        handleStreamOffline(e.broadcasterId)
      ),
...
async function syncEventSubSubscriptions() {
  ...
  for (const sub of twitchSubs.data) {
    ...
    const broadcasterId =
      sub.condition?.broadcaster_user_id || sub.condition?.user_id;
